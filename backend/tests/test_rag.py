...
@patch("app.rag.pipeline.generation_chain")
@patch("app.rag.pipeline.self_rag_chain")
def test_pipeline_returns_answer(mock_self_rag, mock_generation, mock_retrieve):
    from app.rag.pipeline import run

    mock_retrieve.return_value = [
        Document(
            page_content="Les SRO sont le traitement de la diarrhée.",
            metadata={"protocol": "Diarrhée", "section": "# Traitement", "type": "text"}
        )
    ]
    mock_generation.invoke.return_value = "Administrer des SRO au patient."
    mock_self_rag.invoke.return_value = "Administrer des SRO au patient."
...
@patch("app.rag.pipeline.generation_chain")
@patch("app.rag.pipeline.self_rag_chain")
def test_pipeline_sources_extracted_correctly(mock_self_rag, mock_generation, mock_retrieve):
    from app.rag.pipeline import run
    ...
    mock_retrieve.return_value = [
        ...
        Document(
            ...
            metadata={"protocol": "Diarrhée", "section": "# Symptômes", "type": "text"}
        ),
    ]
    mock_generation.invoke.return_value = "Réponse test."
    mock_self_rag.invoke.return_value = "Réponse test."
